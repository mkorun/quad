import type { HtmlFmtOptions } from './types.js'
...
const isWhitespaceSensitiveTag = (tagName: string): boolean =>
	htmlWhitespaceSensitiveElements.has(tagName) || tagName.indexOf('-') > 0
...
const isWsCode = (code: number): boolean => code === 32 || code === 9 || code === 10 || code === 13
const isTrailingWsCode = (code: number): boolean =>
	isWsCode(code) || code === 11 || code === 12 || code === 160 || code === 5760 ||
...
const isTagNameCode = (code: number): boolean => isLetterCode(code) || (code >= 48 && code <= 57) || code === 45 /* '-' */ || code === 58 /* ':' */
...
const isHtmlWsCode = (code: number): boolean => code === 32 || code === 9 || code === 10 || code === 12 || code === 13
...
const isTrailingSpaceOrNewline = (s: string | undefined): boolean => {
	if (!s) return false
	const c = s.charCodeAt(s.length - 1)
	return c === 32 || c === 10
}

const htmlTrimStart = (s: string): string => {
	let i = 0
	while (i < s.length && isHtmlWsCode(s.charCodeAt(i))) i++
	return i === 0 ? s : s.slice(i)
}

const htmlTrim = (s: string): string => {
	let start = 0
	let end = s.length
	while (start < end && isHtmlWsCode(s.charCodeAt(start))) start++
	while (end > start && isHtmlWsCode(s.charCodeAt(end - 1))) end--
...
const cleanTagText = (text: string): string => {
	...
	let result = text
	if (needsTabFix) result = result.replace(/\t/g, '  ')
	if (needsLeadingFix) {
		result = c1 === 47 ? result.replace(/^<\/\s+/, '</') : result.replace(/^<\s+/, '<')
	}
	const end = result.length - 1
	if (end < 0 || result.charCodeAt(end) !== 62 /* '>' */) return result

	let beforeEnd = end - 1
	while (beforeEnd >= 0 && isTrailingWsCode(result.charCodeAt(beforeEnd))) beforeEnd--
	if (result.charCodeAt(beforeEnd) === 47 /* '/' */) {
		let beforeSlash = beforeEnd - 1
		while (beforeSlash >= 0 && isTrailingWsCode(result.charCodeAt(beforeSlash))) beforeSlash--
	...
	while (i < len) {
		...
		if (html.charCodeAt(i + 1) === 33 && html.slice(i, i + 9).toLowerCase() === '<!doctype') {
			...
			while (j < len) {
				...
				if (c === 34 || c === 39 /* '"' | "'" */) { inQuote = c; j++; continue }
				if (c === 62 /* '>' */) { j++; break }
				j++
			}
		...
		let j = p
		while (j < len && isTagNameCode(html.charCodeAt(j))) j++
...
const normalizeTagAttrs = (str: string): [string, string] => {
	let out = ''
	const len = str.length
	let i = 0
	while (i < len && str.charCodeAt(i) !== CH_SP && str.charCodeAt(i) !== CH_GT) i++
	// Tag name starts at the first letter after '<' — skips a leading '!' for "<!doctype"
	let nameStart = 1
	while (nameStart < i && !isLetterCode(str.charCodeAt(nameStart))) nameStart++
	const tagName = str.slice(nameStart, i).toLowerCase()
	out += str.slice(0, nameStart).toLowerCase() + tagName
	while (i < len) {
		const c = str.charCodeAt(i)
		if (c === CH_GT || (c === CH_SLASH && str.charCodeAt(i + 1) === CH_GT)) { out += str.slice(i); break }
		if (c === CH_SP) { out += ' '; i++; while (i < len && str.charCodeAt(i) === CH_SP) i++; continue }
		if (c === CH_SLASH) {
		...
		while (j < len) {
			...
			if (cc === CH_EQ || cc === CH_SP || cc === CH_GT || cc === CH_SLASH) break
			j++
		}
		out += str.slice(i, j); i = j
		if (i >= len || str.charCodeAt(i) !== CH_EQ) continue
		out += '='; i++
		const q = str.charCodeAt(i)
		if (q === CH_DQ) {
			j = i + 1
			while (j < len && str.charCodeAt(j) !== CH_DQ) j++
			out += '"' + str.slice(i + 1, j) + '"'; i = j + 1
		} else if (q === CH_SQ) {
			j = i + 1
			while (j < len && str.charCodeAt(j) !== CH_SQ) j++
			out += '"' + str.slice(i + 1, j).replace(/"/g, '&quot;') + '"'; i = j + 1
		} else {
			j = i
			while (j < len) {
				const cc = str.charCodeAt(j)
				if (cc === CH_SP || cc === CH_GT) break
				j++
			}
			out += '"' + str.slice(i, j) + '"'; i = j
		}
...
const closingTagName = (str: string): string => {
	const len = str.length
	let i = 2 // skip "</"
	while (i < len && str.charCodeAt(i) !== CH_SP && str.charCodeAt(i) !== CH_GT) i++
...
const multilines = (lines: string[], paddingLeft: number, lineWrap: number) => {
	const result = []
...
const lastLineLength = (s: string): number => {
	const i = s.lastIndexOf('\n')
	return i === -1 ? s.length : s.length - i - 1
}
const firstLineLength = (s: string): number => {
	const i = s.indexOf('\n')
	return i === -1 ? s.length : i
}

const htmlFmt = (html: string, options: HtmlFmtOptions = {}) => {
	if (html.length < 1) {
		return html
	}
	...
	const pad = (n: number): string => {
		while (pads.length <= n) pads.push(indentation.repeat(pads.length))
		return pads[n]!
	}
	...
	for (let i = 0, ln = parts.length; i < ln; i++) {
		...
		if (i % 2 != 0) {
			...
			const attachesToPrevious = isWhitespaceSensitive && (
				(previousRaw.length > 0 && !isHtmlWsCode(previousRaw.charCodeAt(previousRaw.length - 1))) ||
			...
			if (isClosing) {
				...
				if (isComment) {
					if (i > 1 && !prevBoundarySensitive) out.push('\n')
					const last = out[out.length - 1]?.at(-1) ?? ''
					const atLineStart = last === '\n'
					out.push(
						(atLineStart ? pad(indentationCount) : prevBoundarySensitive && prevTextTrailingWs && last !== ' ' ? ' ' : '') + str,
					)
				} else if (str.length > lineWrap) {
					const tagParts = str.split(/" /g)
					out.push(
						(attachesToPrevious ? '' : pad(indentationCount)) +
						tagParts.join(
			...
			if (!str) {
				if (prevTag === '--' && prevTagType === 'opening') {
					...
					if (isWhitespaceSensitiveTag(nextTag)) out.push(' ')
				}
				prevIndentLength = pad(indentationCount).length
				...
				if (prevTag !== 'script' || htmlTrim(str) !== '') {
					...
					out.push(content)
					if (prevTag === 'script') prevContent = str  // truthy → closing-tag handler will indent </script>
				}
				continue
			...
			if (
				...
				if (firstSpace === -1) {
					...
					const rest = multilines(
						[flat.slice(firstSpace + 1)],
						pad(indentationCount).length,
						lineWrap,
					) ?? []
					out.push(
						flat.slice(0, firstSpace) +
				...
				const content =
					multilines(lines, pad(indentationCount).length, lineWrap) ?? []
				out.push(
					'\n' +
					pad(indentationCount) +
					content.join('\n' + pad(indentationCount)) +
